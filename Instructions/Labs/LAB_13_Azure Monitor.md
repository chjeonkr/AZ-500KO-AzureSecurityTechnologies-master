...
- 실습 1: Azure Monitor를 사용하여 Azure 가상 머신에서 데이터 수집
...
1. **Log Analytics 작업 영역** 블레이드에서 **+ 새로 만들기**를 클릭합니다.
...
1. **작업 영역 데이터 원본** 섹션에서 Log Analytics 작업 영역 블레이드에서 **가상 머신** 항목을 클릭합니다.

    >**참고**: 에이전트를 성공적으로 설치하려면 가상 머신이 실행되어야 합니다.

1. 가상 머신 목록에서 이 연습의 첫 번째 작업에 배포한 Azure VM **myVM**을 나타내는 항목을 찾습니다. 이것은 **연결되지 않음**으로 나열되어 있습니다.
...
1. **에이전트 구성** 블레이드에서 Windows 이벤트 로그, Windows 성능 카운터, Linux 성능 카운터, IIS 로그 및 Syslog를 포함한 구성 가능한 설정을 검토합니다. 

1. **Windows 이벤트 로그**가 선택되어 있는지 확인하고 **+ Windows 이벤트 로그 추가**를 클릭한 다음 이벤트 로그 유형 목록에서 **시스템**을 선택하고 **+** 를 클릭합니다.

    >**참고**: 이렇게 작업 영역에 이벤트 로그를 추가합니다. 기타 선택 사항으로는 **하드웨어 이벤트** 또는 **키 관리 서비스** 등이 있습니다.  

1. **정보** 체크박스 선택을 취소하고 **오류** 및 **경고** 체크박스를 선택한 상태로 둡니다.

1. **Windows 성능 카운터**를 클릭하고 **+ 성능 카운터 추가**를 클릭한 다음 사용 가능한 성능 카운터 목록을 검토하고 다음 카운터를 추가합니다.
...
1. 필요한 경우 **Log Analytics 시작** 창을 닫습니다. 

1. **쿼리** 창의 **모든 쿼리** 열에서 리소스 유형 목록 아래쪽으로 스크롤하고 **가상 머신**을 클릭합니다.
    
1. 미리 정의된 쿼리 목록을 검토하고, **메모리 및 CPU 사용**을 선택하고, 해당 **실행** 단추를 클릭합니다.
...
1. 쿼리는 새 쿼리 탭에서 자동으로 열립니다. 
    ...
    1. Azure VM 블레이드의 **작업** 섹션에서 **명령 실행**을 선택하고 **명령 스크립트 실행** 블레이드에서 다음 스크립트를 입력하고 **실행**을 클릭합니다.
    2. 
       ```cmd
       cmd
       :loop
    ...
    1. Log Analytics 블레이드로 다시 전환하고 쿼리를 재실행합니다. 데이터가 수집될 때까지 몇 분 기다렸다가 쿼리를 재실행해야 할 수도 있습니다.
