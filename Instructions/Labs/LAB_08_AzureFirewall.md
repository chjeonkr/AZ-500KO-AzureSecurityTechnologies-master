...
1. **방화벽 만들기** 블레이드의 **기본** 탭에서 다음 설정을 지정합니다(다른 설정은 기본값으로 남겨둡니다).
   ...
   |이름|**Test-FW01**|
   |지역|**(미국) 미국 동부**|
   |방화벽 관리|**Firewall 규칙(클래식)을 사용하여 이 방화벽 관리**|
...
1. **경로 테이블** 블레이드에서 **+ 새로 만들기**를 클릭합니다.
...
1. **Test-FW01** 블레이드의 **설정** 섹션에서 **규칙(클래식)** 을 클릭합니다.

1. **Test-FW01 \| 규칙(클래식)** 블레이드에서 **애플리케이션 규칙 컬렉션** 탭을 클릭한 다음 **+ 애플리케이션 규칙 컬렉션 추가**를 클릭합니다.
...
1. **Test-FW01 \| 규칙(클래식)** 블레이드에서 **네트워크 규칙 컬렉션** 탭을 클릭한 다음 **+ 네트워크 규칙 컬렉션 추가**를 클릭합니다.
...
1. **네트워크 규칙 컬렉션 추가** 블레이드에서 다음 설정을 사용하여 **IP 주소** 섹션에 새 항목을 만듭니다. 다른 설정은 기본값으로 유지합니다.
...
1. **`http://www.microsoft.com/`** 으로 이동합니다.
    ...
    >**참고**: 브라우저 페이지 내에서 다음과 유사한 텍스트가 포함된 메시지를 받아야 합니다. `HTTP request from 10.0.2.4:xxxxx to microsoft.com:80. Action: Deny. No rule matched. Proceeding with default action.` 이 현상은 방화벽이 이 웹 사이트에 대한 액세스를 차단하기 때문에 발생할 것으로 예상됩니다. 
